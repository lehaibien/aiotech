...
-- =============================================
CREATE PROCEDURE [dbo].[usp_SaleReport_Get]
  @iStartDate DATE,
  @iEndDate DATE,
  @iCategory NVARCHAR(4000),
  @iProduct NVARCHAR(4000)
AS
BEGIN
  WITH
    Months
    ...
    (
      ...
      Ord.CreatedDate BETWEEN @iStartDate AND @iEndDate
        AND (ISNULL(@iProduct, '') = '' OR Prd.Name LIKE '%' + @iProduct + '%')
        AND (ISNULL(@iCategory, '') = '' OR Cat.Name LIKE '%' + @iCategory + '%')
      GROUP BY 
