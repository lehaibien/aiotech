...
type BrandToolbarProps = {
  selectedRows: BrandResponse[];
  onSearch: (searchTerm: string) => void;
};

export const BrandToolbar = ({ selectedRows, onSearch }: BrandToolbarProps) => {
  const handleDelete = async () => {
    ...
    const response = await deleteListApi(API_URL.brand, ids);
    if (response.success) {
      notifications.show({
        title: "Hệ thống",
