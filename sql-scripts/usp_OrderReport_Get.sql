...
GO -- =============================================
    ...
    -- =============================================
CREATE
OR ALTER PROCEDURE [dbo].[usp_OrderReport_Get] @iStartDate DATE,
@iEndDate DATE,
@iCustomerUsername NVARCHAR(4000) AS BEGIN
SET NOCOUNT ON;
WITH Months AS (
...
SalesData AS (
    ...
    WHERE Ord.CreatedDate BETWEEN @iStartDate AND @iEndDate
        AND (
            ISNULL(@iCustomerUsername, '') = ''
            OR Usr.Username LIKE '%' + @iCustomerUsername + '%'
        )
